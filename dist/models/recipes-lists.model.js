...
const findRecipesByGroceryListId = (id) => __awaiter(void 0, void 0, void 0, function* () {
    const recipeGroceryLists = yield db(recipesGrocerylists)
        .join("recipes", "recipes.id", "=", "recipes-grocery-lists.recipe-id")
        .join("grocery-lists", "grocery-lists.id", "=", "recipes-grocery-lists.grocery-list-id")
        .where("recipes-grocery-lists.grocery-list-id", id)
        .select("recipes-grocery-lists.id as recipes-grocery-lists-id", "recipes.id as recipe-id", "grocery-lists.id as grocery-list-id", "recipes.user-id", "recipes.recipe-name", "grocery-lists.name as grocery-list-name", "recipes.description", "recipes.img-url", "grocery-lists.completed", "recipes-grocery-lists.created_at", "recipes-grocery-lists.updated_at");
    ...
    for (let i = 0; i < recipeGroceryLists.length; i++) {
        ...
        if (recipeGroceryLists[i]["img-url"] !== null)
...
const findGroceryListIdsByUserId = (id) => __awaiter(void 0, void 0, void 0, function* () {
    const ids = yield db(recipesGrocerylists)
    ...
    for (let i = 0; i < reducedIds.length; i++) {
        let result = yield findRecipesByGroceryListId(reducedIds[i]);
...
const findIngredientsByGroceryListId = (id) => __awaiter(void 0, void 0, void 0, function* () {
    const recipeIds = yield findRecipeIdsByGroceryListId(id);
    const combinedIngredients = [];
    for (let i = 0; i < recipeIds.length; i++) {
        const ingredientToAdd = yield findIngredientsByRecipeId(recipeIds[i].id);
